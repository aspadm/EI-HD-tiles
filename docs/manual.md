...
gen_tileset.exe - сборщик атласов
gen_mod.exe - сборщик мода
config.yaml - настройки сборщиков атласов/модов

check_masks.exe - генерация тестовой текстуры из масок
check_tile.exe - генерация тестовой текстуры из базового тайла
...
Для этого просто перетащите .yaml файл на `gen_tileset.exe` - это выдаст необходимое количество текстур с атласами в формате png.
...
Сборка мода выполняется перетаскиванием .yaml файла с описанием мода на `gen_mod.exe` - это выдаст готовый к распространению мод в виде zip файла.
...
Изначально программы представляют из себя скрипты на Python 3, которые отлично работают в Linux. То есть, «запуск из исходников» - это то, как стоит использовать данный инструмент. Сборка в exe файлы проведена для удобства конечного пользователя.
